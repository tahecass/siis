...
public class GraphViz {
	...
	private final static String osName = System.getProperty("os.name")
			.replaceAll("\\s", "");
	private Logger log = Logger.getLogger(GraphViz.class);
	...
	private void loadProperties() {
		log.info("Ejecutando el metodo [ loadProperties ]");
		log.info("Sistema Operativo ==> " + osName);

		if (osName.contains("Linux")) {
			TEMP_DIR = IConstantes.tempDirForMacOSX; // es la misma ruta de mac
			DOT = IConstantes.dotForLinux;
		} else if (osName.contains("MacOSX")) {

			TEMP_DIR = IConstantes.tempDirForMacOSX;
			DOT = IConstantes.dotForMacOSX;

		} else if (osName.contains("Windows")) {
			TEMP_DIR = IConstantes.tempDirForWindows;
			log.info("TEMP_DIR ==> " + TEMP_DIR);
			DOT = IConstantes.dotForWindows;
		}
	}
	...
	public void setDotSource(StringBuilder graph) {
		this.graph = graph;
	}
	...
	public byte[] getGraph(String dot_source, String type,
		...
		log.info("Ejecutando el metodo [getGraph]");
		File dot;
	...
	private byte[] get_img_stream(File dot, String type,
			String representationType) {
		log.info("Ejecutando el metodo [get_img_stream] ");
		File img;
		byte[] img_stream = null;

		try {

			if (!TEMP_DIR.isEmpty() && TEMP_DIR != null) {
	...
	private File writeDotSourceToFile(String str) throws java.io.IOException {
		log.info("Ejecutando el metodo [ writeDotSourceToFile ]");
		File temp = null;
		try {
			if (!TEMP_DIR.isEmpty() && TEMP_DIR != null) {
